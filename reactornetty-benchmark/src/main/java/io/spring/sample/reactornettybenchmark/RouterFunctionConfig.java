...
final class RouterFunctionConfig {
	...
	static BiFunction<HttpServerRequest, HttpServerResponse, Publisher<Void>> exit() {
		return (req, res) -> {
			req.withConnection(conn -> conn.channel().eventLoop().execute(() -> System.exit(0)));
			return res.header("Content-Type", "text/plain")
					.sendObject(Unpooled.wrappedBuffer(exitBytes));
		};
	}
	...
	static final byte[] exitBytes = "Exiting!".getBytes(CharsetUtil.ISO_8859_1);
